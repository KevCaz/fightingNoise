...
###### ![:faic](globe) 23/10/2018 - University of Guelph
...
### 1. I'm post-doctoral fellow

--

### 2. PhD in Biogeography Rimouski/Montpellier

--

### 3. Theory / Statistics / Computer Science
...
![:scalec 44%](img/cardreport.png)

* ![:faic](file-text) Roebuck, K. et al. (2017)
[Canadian's Eating in the Dark](http://www.seachoice.org/wp-content/uploads/2017/03/Seafood-Labelling-Report-Online.pdf)
...
##### - [vessel-sinking policy](https://www.scientificamerican.com/article/blowing-up-illegal-fishing-boats-helps-indonesian-fishers/)
...
##### ![:faic](arrow-right) Well that's what the talk is about

##### ![:faic](arrow-right) We combine biotracers to do so
...
####  ![:faic](angle-double-right) Why should we combine multiple sources of information in this context? How to do so?
...
#### [![:faic](external-link) more details](https://www.alaskankingcrab.com/blogs/resources/blog-learn-fishing-for-king-crab)
...
![:scalec 80%](img/drunkenKC.png)

* ![:faic](file-text) [Drunken Alaska King Crab Legs](https://iowagirleats.com/2013/12/11/drunken-alaska-king-crab-legs-hy-vee-cooking-video/)
...
```{R figA2, echo = FALSE, fig.height = 4.5}
par(mypar)
...
```{R fig0, echo = FALSE, fig.height = 5.4}
n <- 4
par(mypar, mfrow = c(2,1), mar = c(3, 4, 0, 3))
figDens(seqx, ylim = c(0, .8), runif(n, -2, 2), runif(n, .5, 2), pal = colorRampPalette(c(col1, "grey80"))(n), xlab = "Biotracer 1", lwd = 3)
figDens(seqx, ylim = c(0, .8), runif(n, -2, 2), runif(n, .5, 2), pal = colorRampPalette(c(col1, "grey80"))(n), xlab = "Biotracer 2", lwd = 3)
```
